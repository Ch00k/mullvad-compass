...
import (
	...
	"github.com/Ch00k/mullvad-compass/internal/preflight"
...
func TestE2E_FullFlow(t *testing.T) {
	t.Run("Successful execution with results", func(t *testing.T) {
		...
		deps := Dependencies{
			...
			PingLocations: func(_ context.Context, locs []relays.Location, _, _ int, _ relays.IPVersion, _ icmp.SocketType, _ logging.LogLevel) ([]relays.Location, error) {
			...
			CheckICMPPrivileges: func(_ relays.IPVersion, _ logging.LogLevel) preflight.CheckResult {
				return preflight.CheckResult{Passed: true, SocketType: icmp.SocketTypeUDP}
			},
			Stdout: &output,
		}
...
func TestE2E_PreflightChecks(t *testing.T) {
	t.Run("Preflight failure - missing privileges", func(t *testing.T) {
		var output bytes.Buffer
		...
		deps := Dependencies{
			...
			PingLocations: func(_ context.Context, locs []relays.Location, _, _ int, _ relays.IPVersion, _ icmp.SocketType, _ logging.LogLevel) ([]relays.Location, error) {
				t.Error("PingLocations should not be called when preflight checks fail")
				return locs, nil
			},
			ParseRelaysFile: func(_ logging.LogLevel, _ string, _ func() (string, error)) (*relays.File, error) {
				return relays.ParseRelaysFile("../../testdata/relays.json")
			...
			CheckICMPPrivileges: func(_ relays.IPVersion, _ logging.LogLevel) preflight.CheckResult {
				return preflight.CheckResult{
					Passed: false,
					Error: fmt.Errorf(
						"insufficient privileges to create ICMP socket\nTry running with sudo or grant capabilities: sudo setcap cap_net_raw+ep /path/to/mullvad-compass",
					),
				}
			},
			Stdout: &output,
		...
		if err == nil {
			t.Fatal("Expected error when preflight checks fail")
		}
		if !strings.Contains(err.Error(), "insufficient privileges") {
			t.Errorf("Expected privilege error, got: %v", err)
		}
		if !strings.Contains(err.Error(), "sudo") || !strings.Contains(err.Error(), "setcap") {
			t.Errorf("Expected error to contain guidance about sudo or setcap, got: %v", err)
		}
	})

	t.Run("Preflight failure - IPv6 unavailable", func(t *testing.T) {
		var output bytes.Buffer
		...
		deps := Dependencies{
			...
			CheckICMPPrivileges: func(_ relays.IPVersion, _ logging.LogLevel) preflight.CheckResult {
				return preflight.CheckResult{
					Passed: false,
					Error:  fmt.Errorf("IPv6 not available on this system\nCheck network configuration"),
				}
			},
			Stdout: &output,
		}

		args := []string{"--ipv6", "-m", "500"}
		err := run(context.Background(), args, deps)

		if err == nil {
			t.Fatal("Expected error when IPv6 preflight check fails")
		}
		if !strings.Contains(err.Error(), "IPv6 not available") {
			t.Errorf("Expected IPv6 unavailable error, got: %v", err)
		}
		if !strings.Contains(err.Error(), "network configuration") {
			t.Errorf("Expected error to mention network configuration, got: %v", err)
		}
	})

	t.Run("Preflight success - socket type passed to PingLocations", func(t *testing.T) {
		var output bytes.Buffer
		var receivedSocketType icmp.SocketType
		...
		deps := Dependencies{
			...
			PingLocations: func(_ context.Context, locs []relays.Location, _, _ int, _ relays.IPVersion, socketType icmp.SocketType, _ logging.LogLevel) ([]relays.Location, error) {
				receivedSocketType = socketType
				for i := range locs {
					latency := 15.0
					locs[i].Latency = &latency
				}
		...
		if receivedSocketType != icmp.SocketTypeRaw {
			t.Errorf("Expected socket type Raw to be passed to PingLocations, got: %v", receivedSocketType)
		}
	})
}
