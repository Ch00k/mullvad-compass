...
func TestRunPreflightChecks(t *testing.T) {
	t.Run("Returns success when check passes", func(t *testing.T) {
		socketType, err := runPreflightChecks(
			relays.IPv4,
			logging.LogLevelError,
			func(relays.IPVersion, logging.LogLevel) preflight.CheckResult {
		...
		if socketType != icmp.SocketTypeRaw {
			t.Errorf("Expected SocketTypeRaw, got %v", socketType)
		}
	})
	...
	t.Run("Returns error when check fails", func(t *testing.T) {
		...
		if socketType != icmp.SocketTypeUDP {
			t.Errorf("Expected SocketTypeUDP fallback, got %v", socketType)
		}
	})
	...
	t.Run("Logs timing at debug level", func(t *testing.T) {
		...
		_, _ = runPreflightChecks(
			...
			logging.LogLevelDebug,
			func(relays.IPVersion, logging.LogLevel) preflight.CheckResult {
				return preflight.CheckResult{Passed: true}
			},
		)

		logOutput := logBuf.String()
		if !strings.Contains(logOutput, "Preflight check completed in") {
			t.Errorf("Expected timing log message, got: %s", logOutput)
		}
	})
...
func TestPingLocations(t *testing.T) {
	t.Run("Logs timing at debug level", func(t *testing.T) {
		...
		mockPingFn := func(_ context.Context, locs []relays.Location, _, _ int, _ relays.IPVersion, _ icmp.SocketType, _ logging.LogLevel) ([]relays.Location, error) {
			return locs, nil
		}
