...
func getLocations(
	...
	locations, skipped, err := relays.GetLocations(relaysData, serverType, wireGuardObfuscation, daita, ipVersion)
	if err != nil {
		return nil, err
	}
...
// runPreflightChecks performs system capability checks before pinging
func runPreflightChecks(
	ipVersion relays.IPVersion,
	logLevel logging.LogLevel,
	checkFn func(relays.IPVersion, logging.LogLevel) preflight.CheckResult,
) (icmp.SocketType, error) {
	start := time.Now()
	defer func() {
		if logLevel <= logging.LogLevelDebug {
			elapsed := time.Since(start)
			log.Printf("Preflight check completed in %v", elapsed)
		}
	}()

	icmpCheck := checkFn(ipVersion, logLevel)
	if !icmpCheck.Passed {
		return icmp.SocketTypeUDP, icmpCheck.Error
	}

	return icmpCheck.SocketType, nil
}
...
func pingLocations(
	...
	pingLocationsFn func(context.Context, []relays.Location, int, int, relays.IPVersion, icmp.SocketType, logging.LogLevel) ([]relays.Location, error),
) ([]relays.Location, error) {
	start := time.Now()
	defer func() {
		if logLevel <= logging.LogLevelDebug {
	...
	return pingLocationsFn(ctx, locations, timeout, workers, ipVersion, socketType, logLevel)
}
