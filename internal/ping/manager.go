...
func newSocketManager(ipVersion relays.IPVersion, socketTypeHint icmp.SocketType) (*socketManager, error) {
	var conn *xicmp.PacketConn
	var network string
	var err error

	switch socketTypeHint {
	case icmp.SocketTypeRaw:
		// Skip UDP fallback, go straight to raw ICMP
		conn, network, err = icmp.ListenRaw(ipVersion)
	case icmp.SocketTypeUDP:
		// Skip raw ICMP attempt, go straight to UDP
		conn, network, err = icmp.ListenUDP(ipVersion)
	}
...
func (m *socketManager) Ping(ctx context.Context, ipAddr string, timeout time.Duration) *float64 {
	...
	var dst net.Addr
	switch m.network {
	case icmp.NetworkIPv6UDP:
		addr, err := net.ResolveUDPAddr("udp6", "["+ipAddr+"]:0")
		if err != nil {
			return nil
		}
		dst = addr
	case icmp.NetworkIPv4UDP:
		addr, err := net.ResolveUDPAddr("udp4", ipAddr+":0")
		if err != nil {
			return nil
		}
		dst = addr
	case icmp.NetworkIPv6Raw:
		addr, err := net.ResolveIPAddr("ip6", ipAddr)
		if err != nil {
			return nil
		}
		dst = addr
	default: // NetworkIPv4Raw
