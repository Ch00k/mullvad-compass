...
type MockPingerFactory struct {
	mu                  sync.Mutex
	CreatePingerFunc    func(ipVersion relays.IPVersion, socketType icmp.SocketType) (Pinger, error)
...
func (f *MockPingerFactory) CreatePinger(ipVersion relays.IPVersion, socketType icmp.SocketType) (Pinger, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	if f.CreatePingerFunc != nil {
		return f.CreatePingerFunc(ipVersion, socketType)
	}
